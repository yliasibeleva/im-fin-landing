...
heading1('Часть III. Что на самом деле делает бухгалтер каждый месяц')

para('Собственник, как правило, видит только итог: отчёт сдан, налог уплачен, претензий нет. За этим стоит непрерывная, ежедневная работа — невидимая, пока она выполняется правильно, и очень заметная, когда нет. Ниже — реальный состав этой работы применительно к вашей организации.')

heading2('Банковские выписки — ежедневно')
para('Каждое движение по расчётному счёту необходимо идентифицировать: определить природу операции, привязать к договору и контрагенту, выбрать правильный счёт учёта и период. При нескольких лизинговых платежах в месяц, расчётах с поставщиками комплектующих и покупателями готовой продукции — это десятки строк ежедневно. Ошибка в одной строке тянет за собой неверный НДС, неверный остаток по контрагенту и искажение финансового результата.')

heading2('Первичные документы — каждый входящий и исходящий')
para('Каждый счёт-фактура, накладная, акт, УПД должны быть проверены на корректность оформления, занесены в базу, сопоставлены с оплатой и отражены строго в нужном периоде. В вашем случае — это ещё и разделение: что пришло как сырьё, что ушло как готовая продукция, что является услугой. Ошибка в периоде или в квалификации документа — занижение или завышение налоговой базы, риск доначислений при проверке.')

heading2('НДС — ежеквартально, с нулевой терпимостью к ошибкам')
para('Декларация по НДС автоматически сверяется с данными всех контрагентов через систему АСК НДС-2. Любое расхождение — автоматический запрос от налоговой. Бухгалтер обязан выявить и закрыть все расхождения до подачи декларации. При наличии комплектации — входящий НДС по сырью нужно правильно распределить и принять к вычету в момент, когда готовая продукция реализована. Это требует точного учёта движения номенклатуры на каждом этапе.')

heading2('Налог на прибыль — параллельный учёт')
para('Параллельно с бухгалтерским ведётся отдельный налоговый учёт. Разницы между ними необходимо отслеживать, документировать и правильно отражать — иначе декларация по прибыли не сойдётся с балансом. В вашем случае дополнительный источник разниц — лизинговое имущество: бухгалтерский и налоговый учёт амортизации и платежей по ФСБУ 25/2018 принципиально различаются.')

heading2('Лизинг — отдельный участок учёта')
para('По каждому лизинговому договору ежемесячно: разноска платежа по компонентам (погашение обязательства + процентный расход + НДС), начисление амортизации предмета аренды, контроль соответствия графику лизингодателя. При нескольких объектах одновременно — это самостоятельный блок работы, требующий знания ФСБУ 25/2018 и опыта его применения. Именно этот участок в вашей базе не велся с 2020 года — что и стало одним из ключевых поводов для восстановления.')

heading2('Комплектация — учёт производственного типа')
para('Поскольку организация продаёт не то, что покупает, бухгалтер ведёт учёт по производственной логике: поступление сырья → передача в производство → формирование себестоимости готовой продукции → реализация. Каждый этап — отдельные проводки, отдельная номенклатура, отдельный контроль остатков. Отрицательные остатки по счетам 10 и 41, выявленные в ходе диагностики, — прямое следствие того, что эта цепочка не отслеживалась.')

heading2('Зарплата и кадры — ежемесячно, без права на ошибку')
para('Расчёт заработной платы, начисление НДФЛ и страховых взносов, отражение в базе, выплата, отчётность в СФР и налоговую. Любой кадровый факт — приём, увольнение, отпуск, больничный — порождает отдельный документооборот. Задолженность по счёту 70, числящаяся с 2024 года, свидетельствует о том, что этот участок требовал, но не получал должного внимания.')

heading2('Закрытие месяца — итог всей работы')
para('В конце каждого месяца проводятся регламентные операции: амортизация, списание расходов, закрытие затратных счетов 20, 26, 44, формирование финансового результата. Незакрытые счета 26 и 44 с 2021 года в вашей базе означают, что эта операция не выполнялась годами — и финансовый результат всё это время был недостоверным.')

heading2('Отчётность — жёсткие сроки, штрафы за нарушение')
para('Ежеквартально и ежегодно: декларации по НДС и налогу на прибыль, расчёты по страховым взносам, 6-НДФЛ, персонифицированные сведения, бухгалтерская отчётность, статистические формы. Просрочка — штраф или блокировка расчётного счёта.')

heading2('Взаимодействие с налоговой')
para('Запросы, требования, вызовы на комиссии, сверки, встречные проверки по контрагентам — всё это приходит бухгалтеру. От скорости и качества ответа зависит, останется ли ситуация рабочим моментом или перерастёт в выездную проверку. Квалифицированный бухгалтер отвечает так, чтобы не создавать новых вопросов.')

note_box(
    'Итог для собственника:',
    'бухгалтерское сопровождение — это не «сдать отчёт раз в квартал». Это ежедневный контроль финансовых потоков, налоговых обязательств и юридической чистоты каждой операции. Цена ошибки — доначисления, штрафы, блокировка счёта и субсидиарная ответственность руководителя. Цена качественного сопровождения — предсказуемость, спокойствие и возможность принимать решения на основе достоверных данных.',
    '1a3a5c'
)
